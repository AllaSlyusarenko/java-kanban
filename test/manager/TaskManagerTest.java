...
public abstract class TaskManagerTest<T extends TaskManager> {
    ...
    void createNewEpic() {
        ...
        assertEquals(epic, taskManager.getEpicById(epic.getId()), "Возвращает неверный эпик");
    ...
    void createNewSubtask() {
        Epic epic = new Epic("Epic 1", "Description 1");
        taskManager.createNewEpic(epic);
        Task task = new Task("Task 1", "Description 1", 5, LocalDateTime.of(2023, 1, 31, 21, 10));
        taskManager.createNewTask(task);
        Subtask subtask = new Subtask("Subtask 1", "Description 1", 5, LocalDateTime.of(2023, 1, 31, 21, 20), 1);
        taskManager.createNewSubtask(subtask);
        assertEquals(1, taskManager.getAllSubtasks().size(), "Сабтаск не добавлен из-за пересечения по времени");
        ...
        Subtask subtask3 = new Subtask("Subtask 3", "Description 3", 5, LocalDateTime.of(2023, 1, 31, 21, 35), 1);
    ...
    void getTaskById() {
        ...
        Task task2 = new Task("Task 1", "Description 1", 10, LocalDateTime.of(2023, 1, 31, 21, 31));
        taskManager.createNewTask(task);
        assertEquals(task, taskManager.getTaskById(task.getId()), "6.Возвращает не ту задачу");
        taskManager.createNewTask(task2);
        assertEquals(task2, taskManager.getTaskById(task2.getId()), "6.Возвращает не ту задачу");
    ...
    void getEpicById() {
        Epic epic = new Epic("Epic 1", "Description 1");
        taskManager.createNewEpic(epic);
        assertEquals(epic, taskManager.getEpicById(epic.getId()), "Получен неверный эпик");
    ...
    void getSubtaskById() {
        ...
        Subtask subtask = new Subtask("Subtask 1", "Description 1", 10, LocalDateTime.of(2023, 1, 31, 21, 20), 1);
        taskManager.createNewSubtask(subtask);
        //assertEquals(subtask, taskManager.getSubtaskById(subtask.getId()), "Неверный сабтаск");
    ...
    void updateTask() {
        ...
        assertNull(taskManager.getTaskById(task3.getId()), "Нет задачи с таким id");
    }
    ...
    void updateEpic() {
        ...
        assertNotEquals(epic1_1, taskManager.getEpicById(epic.getId()), "Задачи должны быть разными");
